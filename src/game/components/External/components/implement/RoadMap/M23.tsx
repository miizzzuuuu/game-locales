...
export class BaseV2Roadmap extends React.Component<Props> {
	...
	shouldComponentUpdate(nextProps: Readonly<Props>) {
		...
		if (current.history !== next.history ||
			...
			current.baccaratDarkMode !== next.baccaratDarkMode ||
	...
	backTextFragment() {
		...
		return (null
			...
			//                 : type === "big-eye-road"
			//                     ? <span>Big Eye Road</span>

			//                     : type === "small-road"
			//                         ? <span>Small Road</span>

			//                         : type === "cockroach-road"
			//                             ? <span>Cockroach Road</span>
							...
							<Fragment key={`${row}-${rowIdx}-${col}-${colIdx}`}>
...

								{
									this.props.expanded || this.props.small || basePcode === "m34" ?
										...
										: null
									// <CardTooltip
									// 	cardset={ this.props.history[ colIdx * 6 + rowIdx ] }
	...
	bigRoadFragment() {
		...
		//     nary(maxRow).map((row) => {
		//         return (
		//             <div className="row" key={self.crypto.randomUUID()}>
		//                 {
											...
											[
												...
												[col - 1 + this.firstRoadmapDisplayedCol] ?
													<>
														<rect
															...
															x={4.5 + (12 * (col - 1 + this.firstRoadmapDisplayedCol))}
															y={4.5 + 12 * (row - 1)}
															// @ts-ignore
												...
												this.roadmapTiesDisplay[row - 1][col - 1 + this.firstRoadmapDisplayedCol] ? <>
													<path
														...
														strokeLinecap="round"
														// @ts-ignore
														d={`M${7 + (12 * (col - 1 + this.firstRoadmapDisplayedCol))} ${11 + (12 * (row - 1))}l4-4`}
													></path>
													<text
														fill="#000"
														fontFamily="Manrope"
...
export default class M23 extends BaseV2Roadmap {
	dragonPrediction: JSX.Element[] = [];
	tigerPrediction: JSX.Element[] = [];
	...
	render(): any {
		...
		switch (this.props.type) {
			...
			case "predictions":
				...
				const redElements = [
					<div key="roadmap_circle_hollow_dragon" className="sl-roadmap-circle-hollow sl-roadmap-blue" />,
					<div key="roadmap_circle_dragon" className="sl-roadmap-circle sl-roadmap-blue" />,
					<div key="roadmap_slash_dragon" className="sl-roadmap-slash sl-roadmap-blue" />
				];
				const blueElements = [
					<div key="roadmap_circle_hollow_tiger" className="sl-roadmap-circle-hollow sl-roadmap-red" />,
					<div key="roadmap_circle_tiger" className="sl-roadmap-circle sl-roadmap-red" />,
					<div key="roadmap_slash_tiger" className="sl-roadmap-slash sl-roadmap-red" />
				];
