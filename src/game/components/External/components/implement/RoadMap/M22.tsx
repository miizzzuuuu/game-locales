import { checkLastIdx } from "./BaccaratRoadmaps";
    ...
    this.simpleBigRoad!.forEach((item, idx) => {
      if (item === "T") this.pushTieDisplay(this.roadmapTiesDisplay!);
        ...
        const data = {
          ...
          color: item === "B" ? redColor : item === "P" ? blueColor : null,
        ...
        redElement = (props) => (
          <path
            ...
            d={`M${
              196.75 +
              5 * props.col +
              2 * (props.col >= 2 ? Math.floor(props.col / 2) : 0)
            } ${
              68.25 +
              5 * props.row +
              2 * (props.row >= 2 ? Math.floor(props.row / 2) : 0)
            }l3.5-3.5`}
          ></path>
        );
        blueElement = (props) => (
