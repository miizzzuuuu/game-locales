export class StringUtility {
    ...
    public static convertGameCodeToGameName(input: string): string {
        ...
        if (input.length === 3) {
            return fixedPart.split(" ").map((letter: string)=>capitalizeFirstLetter(letter)).join("");
        }
        ...
        return fixedPart.split(" ").map((letter: string)=>capitalizeFirstLetter(letter)).join("") + lastChar;
    }
